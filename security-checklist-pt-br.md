[Voltar ao Guia do Conteúdo](README.md)


### Checklist de Segurança

##### SERVIDORES DE AUTENTICAÇÃO (Cadastro/Login/Autenticação por 2 fatores/Recuperar senha) 
- [ ] Utilizar HTTPS em todos os locais.
- [ ] Armazenar senhas em hashes utilizando `Bcrypt` (salt não é necessário - o `Bcrypt` faz isso para você).
- [ ] Destruir o identificador da sessão após o `logout`.
- [ ] Destuir todas as sessões ativas após a recuperação da senha (ou oferecer para realizar isso).
- [ ] Precisa ter um parâmetro de `estado` no OAuth2.
- [ ] Nenhum redirecionador aberto após login feito com sucesso ou em qualquer outro redirecionamento intermediário.
- [ ] Ao fazer o parsing do input do Cadastro/Login, sanitizar para javascript://, data://, 
